...
function DashboardPage() {
  ...
  const cards = [
    { key: "totalReports" as const, val: counts.total, icon: FileText, tone: "var(--color-brand-mid)" },
    { key: "pending" as const, val: counts.pending, icon: Clock, tone: "var(--color-status-pending)" },
    { key: "assigned" as const, val: counts.assigned, icon: UserCheck, tone: "var(--color-status-assigned)" },
          ...
          >
            <div className="flex items-start justify-between">
              <div
                ...
                style={{ background: `color-mix(in oklab, ${c.tone} 15%, transparent)`, color: c.tone }}
              >
            ...
            <p className="text-xs text-muted-foreground mt-1">{t(c.key)}</p>
          </div>
        ))}
      </section>
